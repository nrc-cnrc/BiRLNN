def preprocess_data(filename_in=None, filename_out=None, model_type='BIMODAL', starting_point='fixed',
                    invalid=True, duplicates=True, salts=True, stereochem=True, canonicalize=True, min_len=34,
                    max_len=74, augmentation=1, base_path=None):
    ...
    base_path = base_path or os.path.abspath(os.path.join(os.path.dirname(__file__), '..'))
    if filename_in is None:
        filename_in = os.path.join(base_path, 'data', 'chembl_smiles')
    ...
    if model_type == "ForwardRNN":
        ...
        if augmentation > 1 and starting_point == 'fixed':  # removes augmentation for fixed starting point
            augmentation = 1

    p.preprocess(name, aug=augmentation, length=max_len)

    if not filename_out:
        filename_out = os.path.join(base_path, 'data', dataname + '_' + name + '.csv')
